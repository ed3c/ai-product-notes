...
class ContractHistoryCorpusTests(unittest.TestCase):
    ...
    def test_registry_breakage_remains_counted(self) -> None:
        ...
        self.assertEqual(
            ("PASS", "FAIL"),
            (r["old_result"]["status"], r["new_result"]["status"]),
        )
    ...
    def test_sdk_downstream_migration_is_counted_with_exact_reasons(self) -> None:
        ...
        self.assertEqual(
            "request_options", r["new_result"]["reasons"][0]["historical_role"]
        )
        self.assertEqual(
            "omitted_optional_placeholder",
            r["new_result"]["reasons"][1]["historical_role"],
        )
    ...
    def test_sdk_receipt_binds_lock_packages_and_exact_ci(self) -> None:
        ...
        self.assertEqual(
            "ff88581e741c79cfbb5f6ddb827b90f39447be71",
            summary["old_ci_head_sha"],
        )
        ...
        self.assertEqual(
            "@modelcontextprotocol/sdk", case["old_contract"]["package_identity"]
        )
        self.assertEqual(
            "@modelcontextprotocol/client", case["new_contract"]["package_identity"]
        )
        self.assertEqual(
            "sdk-major-package-migration", case["old_contract"]["migration_kind"]
        )
    ...
    def test_receipt_is_byte_reproducible_and_digests_are_separate(self) -> None:
        ...
        for r in self.compiled["receipts"]:
            self.assertEqual(
                ...
                set(r["source_digests"]),
            )
            self.assertEqual(
                {"old_contract", "new_contract"}, set(r["contract_digests"])
            )
    ...
    def test_sdk_provenance_failures_block_counting(self) -> None:
        mutations = {
            ...
            "job": lambda c: c["validation"]["old_ci"]["jobs"].update(
                {"TypeScript Typecheck": "skipped"}
            ),
            "version": lambda c: c["validation"]["dependency_lock"].update(
                version="1.28.0"
            ),
            "package": lambda c: (
                c["new_contract"].update(
                    package_identity=c["old_contract"]["package_identity"]
                ),
                c["validation"]["new_package_manifest"].update(
                    package=c["old_contract"]["package_identity"]
                ),
            ),
        }
    ...
    def test_two_argument_new_consumer_is_not_reported_as_historical_breakage(self) -> None:
        ...
        r = next(
            x for x in compile_manifest(manifest)["receipts"] if x["adapter"] == "sdk-api"
        )
        self.assertEqual("PASS", r["new_result"]["status"])
        self.assertEqual("INCONCLUSIVE", r["decision"])
        self.assertFalse(r["counts_toward_real_breakage_gate"])

    def test_roadmap_keeps_base_two_and_aggregate_three_of_five(self) -> None:
