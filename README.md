...
The repository keeps its Top-100 datasets and bilingual research corpus, but the product direction is no longer “collect more products.” The core loop is:
...
- **First external wedge:** Vendor API Blast-Radius CI — map a provider/API change to the exact repository call sites that are exposed to it.
...
- **Roadmap rule:** no customer and paid-pilot receipts means no promotion to `BUILD`.
...
│       └── STACKED_PRS.md            # original graph, PRs, SHAs and merge receipts
├── data/
...
| `FRESHNESS_VERIFIED` | `docs/DAILY_MONITOR_PROMPT.md`, signal fixtures | event date and source date | freshness decision | stale/re-indexed/ambiguous sources are excluded |
...
+ gap ledger
+ deterministic score
+ BUILD | VALIDATE | WATCH | REJECT | BLOCKED
...
- `UNKNOWN`, `CONDITIONAL` and `REJECT` remain visible but cannot silently improve the score.
...
## Historical Stack PR graph and merged delivery

The original Git Town-compatible graph was published as Draft PRs and is retained as historical evidence:
...
Human Admit was applied bottom-up with merge commits so original stack ancestry remains auditable:

| Order | PR | Scope | Merge commit |
|---:|---|---|---|
| 1 | [#7](https://github.com/ed3c/ai-product-notes/pull/7) | governance, Agent routing, State Machines, data flow, Git profile | `bfe03383e96183d6b6eebd24462742090c733811` |
| 2 | [#8](https://github.com/ed3c/ai-product-notes/pull/8) | compiler, schemas, asset rights, portfolio envelopes, tests | `3fc46a25cc0089e092373b1a0c92a0780f91a5a2` |
| 3 | [#9](https://github.com/ed3c/ai-product-notes/pull/9) | first opportunity, MVP, experiments, portfolio handoff, roadmap | `fe7e03557f07b7c9ae91210d0405745b870dafcc` |
| 4 | [#11](https://github.com/ed3c/ai-product-notes/pull/11) | exact-head vs synthetic-merge CI evidence | `0e2654f6a89c6110728950161d968b233c7e96b4` |

See [`docs/git/STACKED_PRS.md`](docs/git/STACKED_PRS.md) for original bases, immutable stage heads, retargeting and hosted receipts.

## Hosted CI receipt

Final retargeted PR #11 run `31881831160` separated the two proof subjects:

```text
exact-head
  subject: e37bf18dd39f91f207753d6aaad546125b62a6f1
  repository contract: PASS
  unit tests: 24 PASS
  opportunity packet reproduction: PASS

synthetic-merge
  subject: 3fad2f88232fc489bc1cf0af4a68d2779944451b
  base parent: fe7e03557f07b7c9ae91210d0405745b870dafcc
  head parent: e37bf18dd39f91f207753d6aaad546125b62a6f1
  parent-lineage assertion: PASS
  repository contract: PASS
  unit tests: 24 PASS
  opportunity packet reproduction: PASS
```

Historical runs remain subject-bound:

- `31878162441`: exact stage head `5b646ec6fe70dd2047734636b8dfd517ee2998b2`, synthetic merge `3bb417881393b5faad2a91056c49c77eefeb3cc8`, base `83243ba32729a75e953125370a8cb0b61cee197f`.
- `31878346277`: pre-retarget terminal head `ac66cc60963b77c5f5872d0825c40319c2bfa855` and its generated merge subject.

These are `HOSTED_VERIFIED` receipts for their exact subjects only. They do not prove live Git Town, buyer demand, production safety or revenue.
...
The control plane is merged. The next admissible work is evidence generation, not feature expansion:
